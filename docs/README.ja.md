...
静的 HTML/JS/CSS + `data.json` を出力します。HTTP サーバーで配信すればそのまま動作します。
...
モノレポ構成で 3 つのパッケージに分離されています。

```
packages/
  core/    # @md-meta-view/core  — 型定義、MD パーサー、設定ローダー
  cli/     # md-meta-view        — CLI エントリ、dev サーバー、ビルドコマンド
  web/     # @md-meta-view/web   — React フロントエンド
```
